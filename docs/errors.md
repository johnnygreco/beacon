...
For install and reset/replay operations, see the CLI help and
[ClickHouse schema notes](clickhouse.md).
...
- Capture stores parse and capture failures in `capture_errors` with source
  coordinates and logs watcher or batcher failures with source context.
